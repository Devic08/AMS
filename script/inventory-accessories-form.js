...
const accessoryForm = document.getElementById("accessoryModelForm");
const fieldName = document.getElementById("accessoryNameInput");
const fieldImageUrl = document.getElementById("accessoryImageInput");
const fieldImageUpload = document.getElementById("accessoryImageUploadInput");
const fieldImageUploadName = document.getElementById("accessoryImageUploadName");
const fieldType = document.getElementById("accessoryTypeInput");
const fieldBrand = document.getElementById("accessoryBrandInput");
const fieldEolYears = document.getElementById("accessoryEolYearsInput");
const fieldEolMonths = document.getElementById("accessoryEolMonthsInput");
const fieldEolDays = document.getElementById("accessoryEolDaysInput");
const fieldMinQty = document.getElementById("accessoryMinQtyInput");
const fieldTotalQty = document.getElementById("accessoryTotalQtyInput");
const fieldInvoiceNumber = document.getElementById("accessoryInvoiceNumberInput");
const fieldInvoiceDocumentUpload = document.getElementById("accessoryInvoiceDocumentUploadInput");
const fieldInvoiceDocumentUploadName = document.getElementById("accessoryInvoiceDocumentUploadName");
const formStatus = document.getElementById("accessoryFormStatus");
...
const submitButton = accessoryForm?.querySelector("button[type='submit']");
const formContext = loadFormContext();

function loadFormContext() {
    try {
        const raw = window.sessionStorage.getItem(ACCESSORY_FORM_CONTEXT_KEY);
        if (!raw) {
            return null;
        }
...
function loadStoredRows() {
    try {
        const payload = window.localStorage.getItem(ACCESSORY_STORAGE_KEY);
        if (!payload) {
            return [];
        }
...
function getFallbackValues() {
    const accessories = window.AMSFieldSets?.predefinedFieldOptions?.Accessories || {};
    return {
        accessoryType: toUnique(accessories["Accessory Type"] || ["Keyboard", "Mouse", "Docking Station", "Headset", "Adapter", "Cable"]),
        accessoryBrand: toUnique(accessories["Accessory Brand"] || ["Logitech", "HP", "Dell", "Lenovo", "Belkin"])
    };
}
...
function resetForm() {
    ...
    renderOptions(fieldType, fallback.accessoryType, "Select Accessory Type");
    renderOptions(fieldBrand, fallback.accessoryBrand, "Select Manufacturer");
    if (fieldEolYears) {
        fieldEolYears.value = "";
    }
    if (fieldEolMonths) {
...
function applyFormModeContext() {
    ...
    if (mode === "clone") {
        if (formHeading) {
            formHeading.textContent = "Clone Accessory Model";
        }
        if (formIntro) {
            formIntro.textContent = "Clone an accessory model and change details before saving to avoid duplicate model names.";
        }
        if (submitButton) {
    ...
    if (mode === "edit") {
        if (formHeading) {
            formHeading.textContent = "Edit Accessory Model";
        }
        if (formIntro) {
            formIntro.textContent = "Update the selected accessory model. Saving will update the same row in the inventory table.";
        }
        if (submitButton) {
    ...
    setSelectValue(fieldType, sourceRow.accessoryType);
    setSelectValue(fieldBrand, sourceRow.brand);
...
async function populateFieldValues() {
    ...
    try {
        ...
        const apiTypes = toUnique(groupMap.accessory_type || []);
        const apiBrands = toUnique(groupMap.accessory_brand || []);

        if (apiTypes.length) {
            renderOptions(fieldType, apiTypes, "Select Accessory Type");
            applyFormModeContext();
        }
        if (apiBrands.length) {
            renderOptions(fieldBrand, apiBrands, "Select Manufacturer");
            applyFormModeContext();
        }
    } catch {
...
accessoryForm?.addEventListener("submit", async (event) => {
    ...
    if (!fieldName.value.trim() || !fieldTotalQty.value.trim()) {
        formStatus.textContent = "Accessory model name and total qty are required.";
    ...
    const newRow = {
        id: mode === "edit" && sourceRow?.id ? sourceRow.id : `accessory-model-${Date.now()}`,
        name: fieldName.value.trim(),
        imageUrl: resolvedImageUrl,
        thumb: fieldName.value.trim().slice(0, 2).toUpperCase(),
        accessoryType: fieldType.value || "General",
        brand: fieldBrand.value || "Unknown",
    ...
    formStatus.textContent = mode === "edit"
        ? "Accessory model updated. Returning to inventory..."
        : "Accessory model saved. Returning to inventory...";
    formStatus.classList.add("visible");
    setTimeout(() => {
        window.location.href = "inventory-accessories.html";
    }, 350);
});

accessoryForm?.addEventListener("reset", () => {
    window.setTimeout(() => {
        resetForm();
