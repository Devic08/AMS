...
function renderAccessoryCard(person, assignment, assignmentId) {
    ...
    return `
        <article class="manageUserDetailCard">
            ...
            <div class="manageUserDetailList is-user-section">
                ...
                <div class="manageUserDetailItem"><label>Accessory Model</label><span>${isObject ? (assignment.accessoryModel || assignment.name || assignment.model || "-") : String(assignment || "-")}</span></div>
                <div class="manageUserDetailItem"><label>Manufacturer</label><span>${isObject ? (assignment.manufacturer || assignment.brand || "-") : "-"}</span></div>
                <div class="manageUserDetailItem"><label>Accessory Type</label><span>${isObject ? (assignment.accessoryType || "-") : "-"}</span></div>
                <div class="manageUserDetailItem"><label>Quantity</label><span>${isObject ? (assignment.quantity || 1) : 1}</span></div>
                <div class="manageUserDetailItem"><label>Assigned Date</label><span>${isObject ? (assignment.assignedDate || assignment.start || "-") : "-"}</span></div>
...
function renderNotFound() {
    detailsTitle.textContent = "Accessory Assignment Not Found";
        ...
        <article class="manageUserDetailCard">
            ...
            <div class="manageUserDetailList">
                <div class="manageUserDetailItem">
                    <label>Status</label>
                    <span>The selected accessory assignment could not be found.</span>
                </div>
            </div>
        </article>
    `;
...
function renderDetails() {
    ...
    const assignmentIndex = Number(String(assignmentId).split("-acc-")[1]);
    ...
    detailsTitle.textContent = `${person.name || "User"} Accessory Details`;
    detailGrid.innerHTML = [
        renderUserCard(person),
        renderAccessoryCard(person, assignment, assignmentId)
    ].join("");
}
